...
<BODY>
	...
	try {
		con = DriverManager.getConnection("jdbc:oracle:thin:@127.0.0.1:1521:XE","soldesk", "1234");
		...
		if (!(rs1.next()))
			...
			do {
				...
				out.println("<TD bgcolor=eeeeee>"
						+ rs1.getString("orderdate") + "</TD></TR>");
				out.println("<TR><TD bgcolor=eeeeee>고객 이름</TD>");
				out.println("<TD bgcolor=eeeeee>"
						+ rs1.getString("name") + "</TD></TR>");
				out.println("<TR><TD bgcolor=eeeeee>결제 유형</TD>");
				out.println("<TD bgcolor=eeeeee>");
				out.println(rs1.getString("pay") + "("
						+ rs1.getString("cardno") + ")");
				out.println("</TD></TR>");
				out.println("<TR><TD bgcolor=eeeeee>배달 주소</TD>");
				out.println("<TD bgcolor=eeeeee>"
						+ rs1.getString("addr") + "</TD></TR>");
				...
				if (rs2.next()) {
					...
					do {
						...
						out.println("<TD align=right bgcolor=eeeeee>");
						out.println(nf.format(price * qty)
								+ "</TD></TR>");
					} while (rs2.next());
					out.println("<TR><TD align=right colspan=6>");
					out.println("주문 상품 :" + count + "품목&nbsp;합계금액 :"
							+ totalstr + "원");
