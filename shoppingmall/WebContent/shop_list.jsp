...
<SCRIPT language="javascript">
	function view(temp) {
		if (temp.length > 0) {
			url = "http://127.0.0.1:8080/shoppingmall/upload/" + temp;
			window
					.open(url, "win",
...
<BODY>
	...
	<FORM method=post name=search action="shop_list.jsp">
		<TABLE border=0 width=95%>
			<TR>
				<TH align=right><FONT size=-1>상품명으로 찾기</FONT> <INPUT type=text
					name=pname> <INPUT type=submit value="검색"></font>
				</TD>
			</TR>
			<TR>
				<TH><FONT size=-1> [<A href="main.html">메인으로</A>] <A
						href="shop_list.jsp">전체</A>- <A href="shop_list.jsp?cat=11">가구</A>-
	...
	<CENTER>
		<TABLE width=95% style="font-size: 10pt">
			<%
				...
				String url = "http://127.0.0.1:8080/shoppingmall/upload/";
				...
				try {
					con = DriverManager
							.getConnection("jdbc:oracle:thin:@127.0.0.1:1521:XE", "soldesk","1234");
					...
					if (!(rs.next()))
						...
						for (int j = startrow; j <= endrow; j++) {
							...
							out.println("<A href=JavaScript:view(\""
									+ large.elementAt(j) + "\")>");
							out.println("<IMG border=0 width=70 height=70 src=\""
									+ filename + "\">");
							...
							out.println("시중가: <STRIKE>" + pricestr
									+ "</STRIKE>원&nbsp;&nbsp;");
							...
							if (stocki > 0) {
								...
								out.println("<INPUT type=hidden name=id value="
										+ id + ">");
								out.println("<INPUT type=hidden name=go value="
										+ where + ">");
								out.println("<INPUT type=hidden name=cat value="
										+ ca + ">");
