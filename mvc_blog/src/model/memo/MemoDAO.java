...
/**
 * db와 매우 근접하게 있으면서 db와 소통(sql전달 등...) 한다.
 * 
 * @author soldesk
 *
 */
public class MemoDAO {
	...
	public MemoVO read(int memono) {
		...
		try {
			...
			if (rs.next() == true) {
				// int memono = rs.getInt("memono"); // 컬럼 -> 변수로 변환, DBMS ->
				// JAVA
	...
	public List<MemoVO> list(int sno, int eno, String col, String word) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sql = null; // String 보다 처리속도가 수만배 빠름
		List<MemoVO> list = null;

		sql = new StringBuffer();
		sql.append(" SELECT memono, title, content, wdate, viewcnt, r ");
		sql.append(" FROM( ");
		sql.append(" SELECT memono, title, content, wdate, viewcnt, rownum r ");
		sql.append(" FROM( ");
		sql.append(" SELECT memono, title, content, wdate, viewcnt ");
		sql.append(" FROM memo ");
		...
		try {
			...
			if (word.trim().length() > 0)
				pstmt.setString(++i, word); //검색어가 없을 경우에 건너뜀
			...
			while (rs.next() == true) {
				int memono = rs.getInt("memono"); // 而щ�� -> 蹂���濡� 蹂���, DBMS
													// -> JAVA
				String title = rs.getString("title");
				String content = rs.getString("content");
				String wdate = rs.getString("wdate");
				...
				MemoVO vo = new MemoVO(); // ������ ��肄���瑜� ������ 媛�泥대� 蹂���
				vo.setMemono(memono);
				vo.setTitle(title);
				vo.setContent(content);
				vo.setWdate(wdate);
